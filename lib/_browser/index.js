...
class Result {
    ...
    static try ( res, { allowUndefined, keepError } = {} ) {
        ...
        // Error
            ...
            console.error( Error( `Invalid return value, "Result" object is expected` ) );

            return new this.prototype.constructor( 500 );
        }
    }

    static catch ( e, { silent, keepError } = {} ) {
        var res;
        ...
        // result-like object
            ...
            res = this.exception( [e.status, e.statusText] );
        }
    ...
    static fromJson ( res ) {
        ...
        if ( res instanceof Object && res.constructor === Object ) {
            try {
                if ( res.exception ) {
                    _res = this.exception( [res.status, res.status_text], res.data, res.meta );
                }
                else {
                    _res = this.result( [res.status, res.status_text], res.data, res.meta );
                }
            }
            catch ( e ) {
    ...
    [SET_EXCEPTION] () {
        ...
        if ( this.ok ) {
            this.#status = 500;
            this.#statusText = STATUS_HTTP[this.#status];
        }
    }
}
