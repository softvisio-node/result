...
const STATUS = { ...JSON.parse( fs.readFileSync( module.createRequire( import.meta.url ).resolve( "../resources/status.json" ) ) ), ...JSON.parse( fs.readFileSync( module.createRequire( import.meta.url ).resolve( "../resources/status-websockets.json" ) ) ) };
...
result.exception = Result.exception.bind( Result );
result.tryResult = Result.tryResult.bind( Result );
result.catchResult = Result.catchResult.bind( Result );
result.parseResult = Result.parseResult.bind( Result );
result.getReason = Result.getReason.bind( Result );
